...
from .config import (DISSATISFIED_LABEL, LANGUAGES, MIN_WORDS, MIXED_THRESHOLD,
...
DIACRITICS = re.compile(r"[ً-ٰٟـ]")      # harakat + tatweel
...
def label_from_stars(stars) -> str | None:
    """Weak label. 3-star reviews map to None and are excluded later."""
    try:
        return STAR_TO_LABEL.get(int(stars))
